...
# This line sets the default value for "authenticated" if it hasn't been set already.
st.session_state.setdefault("authenticated", False)

# --- Function to Verify Login ---
def check_password(username, password):
    """
    Checks the username and password using the credentials stored in st.secrets.
    The password stored in st.secrets must be a hashed value generated by pbkdf2_sha256.
    """
    try:
...
if not st.session_state["authenticated"]:
    ...
    if st.button("Login", key="login_button"):
        if check_password(login_username, login_password):
            ...
            st.rerun()  # Rerun the app so that the authenticated view is shown
        else:
            st.error("Incorrect username or password.")

# --- Main Hub (Authenticated View) ---
else:
    # Global CSS styling for the authenticated view
    st.markdown(
        """
        <style>
        /* Sidebar styles */
    ...
    if st.sidebar.button("Logout", key="logout_button"):
        ...
        # Optionally, remove stored login fields
